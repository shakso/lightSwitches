...
Orvibo.prototype.emitIR = function(device, ir, callback) {
  ...
  message = this.prepareMessage({
    ...
    data: {
      ...
      randomA: _.padStart(Math.floor((Math.random() * 255)).toString(16), 2, "0"),
      randomB: _.padStart(Math.floor((Math.random() * 255)).toString(16), 2, "0"),
      // HA HA, OH WOW! This is a doozy. It takes the length of our IR (divided by 2, because bytes)
      // turns it into a hex string, then uses lodash's "padStart" to add leading zeroes if necessary. It then splits the string into chunks of two
      ...
      irlength: this.switchEndian(_.padStart((args.ir.length / 2).toString(16).toUpperCase(), 4, "0")),
      ir: args.ir
    }
  })
  ...
  if (args.device.type == "Socket") {
    message = this.prepareMessage({
      ...
      data: {
        ...
        unknown: "1c00",
        index: this.switchEndian(_.padStart(args.index, 4, "0")),
        ...
        year: this.switchEndian(_.padStart(args.date.year().toString(16), 4, "0")),
        month: this.switchEndian(_.padStart((args.date.month() + 1).toString(16), 2, "0")),
        day: this.switchEndian(_.padStart((args.date.day() + 1).toString(16), 2, "0")),
        hour: this.switchEndian(_.padStart((args.date.hour() + 1).toString(16), 2, "0")),
        minute: this.switchEndian(_.padStart((args.date.minute() + 1).toString(16), 2, "0")),
        second: this.switchEndian(_.padStart((args.date.second() + 1).toString(16), 2, "0")),
        repeat: args.repeat
      }
    })
...
Orvibo.prototype.prepareMessage = function(commandID, macAddress, macPadding, data, callback) {
  ...
  packet = options.magicWord + _.padStart((packet.length / 2).toString(16).toLowerCase(), 4, "0") + args.commandID + args.macAddress + args.macPadding + dataStr
...
Orvibo.prototype.addDevice = function(device) {
  ...
  if (_.filter(this.devices, {
    ...
    if (_.isEqual(args.device, _.filter(this.devices, {
        macAddress: args.device.macAddress
      }))) {
...
Orvibo.prototype.getDevice = function(macAddress, callback) {
  ...
  var device = _.filter(this.devices, {
